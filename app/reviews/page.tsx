...
export default function ReviewsPage() {
  const googleReviewHref =
    businessProfile.googleReviewUrl || "/api/google-review";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <section className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h1 className="font-serif text-4xl sm:text-6xl font-extrabold mt-2">
            Review{" "}
            <span className="text-gold gold-glow-text">
              {businessProfile.name}
            </span>
          </h1>
          <p className="text-white/62 text-base sm:text-lg mt-5 leading-relaxed font-light">
            Your Google review helps new families, couples, brands, and event
            planners find reliable SFX fire show, wedding entry, cold pyro,
            choreography, and event production services.
          </p>
          <ReviewActions
            googleReviewHref={googleReviewHref}
            googleProfileHref={googleProfileHref}
          />
        </section>
          ...
          <div className="rounded-lg bg-charcoal border border-white/5 p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {reviewSteps.map((step, index) => (
                <div
                  key={step}
                  className="p-5 rounded-lg bg-black/35 border border-white/5"
                >
