...
const FALLBACK_EVENT_CATEGORIES = ["Weddings", "Corporate Events", "Dance Sangeet", "School & College", "Cultural"];
...
type ActiveTab = "leads" | "stats" | "contact" | "categories" | "subcategories" | "gallery" | "events";
...
export default function AdminDashboard() {
  ...
  const [contactSettings, setContactSettings] = useState<ContactSettings>(CONTACT_SETTINGS_FORM);
  ...
  const filteredSubcategories = useMemo(
    () => subcategories.filter((item) => getId(item.categoryId) === galleryForm.categoryId),
  ...
  const eventCategoryOptions = useMemo(() => {
    const categoryNames = activeCategories.length > 0 ? activeCategories.map((item) => item.name) : FALLBACK_EVENT_CATEGORIES;
  ...
  const refreshCmsData = useCallback(async () => {
    ...
    try {
      ...
      setFormError("CMS data load nahi ho paaya. Please refresh karke try karein.");
    } finally {
      setLoading(false);
    }
  }, [fetchCategories, fetchContactSettings, fetchEventItems, fetchGalleryItems, fetchHomeStats, fetchSubcategories]);
  ...
  useEffect(() => {
    ...
    if (!firstCategory || (galleryForm.categoryId && subcategoryForm.categoryId)) return;
    ...
    const defaultTimer = window.setTimeout(() => {
      ...
      if (!subcategoryForm.categoryId) {
        setSubcategoryForm((prev) => ({ ...prev, categoryId: firstCategory._id }));
      }
    }, 0);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: "categoryCover" | "subcategoryCover" | "gallery" | "event") => {
    ...
    try {
      ...
      setFormError(getRequestErrorMessage(err, "File upload failed. Cloudinary config check karein."));
    } finally {
  ...
  const saveCategory = async (e: React.FormEvent) => {
    ...
    try {
      const request = editingCategoryId
        ? axios.put("/api/categories", { id: editingCategoryId, ...categoryForm }, { headers: authHeaders })
  ...
  const saveSubcategory = async (e: React.FormEvent) => {
    ...
    try {
      const request = editingSubcategoryId
        ? axios.put("/api/subcategories", { id: editingSubcategoryId, ...subcategoryForm }, { headers: authHeaders })
        : axios.post("/api/subcategories", subcategoryForm, { headers: authHeaders });
      const res = await request;
      if (res.data.success) {
        setFormSuccess(editingSubcategoryId ? "Subcategory updated." : "Subcategory created.");
        setSubcategoryForm({ ...SUBCATEGORY_FORM, categoryId: activeCategories[0]?._id || "" });
        setEditingSubcategoryId(null);
        await fetchSubcategories();
      }
    } catch (err) {
      setFormError(getRequestErrorMessage(err, "Subcategory save nahi ho paayi."));
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const saveGallery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const request = editingGalleryId
        ? axios.put("/api/gallery", { id: editingGalleryId, ...payload }, { headers: authHeaders })
        : axios.post("/api/gallery", payload, { headers: authHeaders });
      const res = await request;
      if (res.data.success) {
        setFormSuccess(editingGalleryId ? "Media item updated." : "Media item published.");
        setGalleryForm({ ...GALLERY_FORM, categoryId: activeCategories[0]?._id || "" });
        setEditingGalleryId(null);
        await fetchGalleryItems();
      }
    } catch (err) {
      setFormError(getRequestErrorMessage(err, "Gallery media save nahi ho paaya."));
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const saveEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await request;
      if (res.data.success) {
        setFormSuccess(editingEventId ? "Event case-study updated." : "Event case-study published.");
  ...
  const updateHomeStat = (index: number, patch: Partial<HomeStatItem>) => {
    setHomeStats((prev) => prev.map((item, itemIndex) => (itemIndex === index ? { ...item, ...patch } : item)));
  };
    ...
    try {
      const payload = {
        stats: homeStats.map((item, index) => ({
          ...item,
          key: item.key || item.label.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""),
          value: Number(item.value) || 0,
          sortOrder: Number(item.sortOrder) || index + 1,
          isActive: item.isActive !== false,
        })),
      };
      const res = await axios.put("/api/home-stats", payload, { headers: authHeaders });
      if (res.data.success) {
        setFormSuccess("Home stats updated.");
        await fetchHomeStats();
      }
    } catch (err) {
      setFormError(getRequestErrorMessage(err, "Home stats save nahi ho paaye."));
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const saveContactSettings = async (e: React.FormEvent) => {
    ...
    clearFeedback();
    try {
      const res = await axios.put("/api/contact-settings", contactSettings, { headers: authHeaders });
      if (res.data.success) {
        setFormSuccess("Contact direct connect updated.");
        await fetchContactSettings();
      }
    } catch (err) {
      setFormError(getRequestErrorMessage(err, "Contact settings save nahi ho paayi."));
    } finally {
      setFormLoading(false);
    }
  };

  const deleteItem = async (endpoint: string, id: string, afterDelete: () => Promise<void>, message: string) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 space-y-5 border-b border-white/5 pb-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
                Leads, media, categories aur event content ko ek clean workspace se manage karein.
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-7">
            {[
              ...
              { label: "Subcategories", value: subcategories.length, icon: Layers3 },
              { label: "Media", value: galleryItems.length, icon: ImageIcon },
              { label: "Events", value: eventItems.length, icon: Sparkles },
            ].map((stat) => {
              const Icon = stat.icon;
              return (
                <div key={stat.label} className="rounded-lg border border-white/5 bg-charcoal/80 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-[11px] font-semibold uppercase tracking-wider text-white/45">{stat.label}</span>
          ...
          <div className="rounded-lg border border-white/5 bg-charcoal/70 p-3 sm:p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-bold text-white">SEO & Content Tools</h2>
                <p className="mt-0.5 text-xs text-white/45">Ranking pages, blogs, packages aur case studies.</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {[
                { href: "/admin/seo", label: "SEO CMS", icon: Sparkles, primary: true },
        ...
        <div className="mb-8 overflow-x-auto pb-1">
          <div className="inline-flex min-w-full gap-2 rounded-lg border border-white/5 bg-charcoal p-1">
            {[
              { key: "leads", label: "Leads", count: inquiries.length, icon: ClipboardList },
              { key: "stats", label: "Stats", count: homeStats.length, icon: Sparkles },
              { key: "contact", label: "Contact", count: 3, icon: Phone },
              { key: "categories", label: "Categories", count: categories.length, icon: FolderTree },
              { key: "subcategories", label: "Subcategories", count: subcategories.length, icon: Layers3 },
              { key: "gallery", label: "Media", count: galleryItems.length, icon: ImageIcon },
              { key: "events", label: "Events", count: eventItems.length, icon: Sparkles },
            ].map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-lg px-3 py-2.5 text-[11px] font-semibold uppercase tracking-wider transition-all sm:text-xs clickable ${
                  activeTab === tab.key ? "bg-gold text-black shadow-lg shadow-gold/10" : "text-white/65 hover:bg-white/5 hover:text-white"
                }`}
              >
                <Icon className="h-4 w-4 shrink-0" />
                <span>{tab.label}</span>
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] ${
                    activeTab === tab.key ? "bg-black/10 text-black" : "bg-white/5 text-white/45"
                  }`}
                >
                  {tab.count}
                </span>
        ...
        {activeTab === "leads" && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold tracking-wide">Client Inquiries</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {inquiries.map((inq) => (
                <div key={inq._id} className="p-6 rounded-lg bg-charcoal border border-white/5">
                  <div className="flex justify-between gap-4 border-b border-white/5 pb-3 mb-4">
                    <div>
                      <h3 className="font-bold">{inq.name}</h3>
                      <a href={`tel:${inq.mobile}`} className="text-gold text-sm font-semibold hover:underline">
                        {inq.mobile}
                      </a>
                    </div>
                  ...
                  <p className="text-white/60 text-sm leading-relaxed mb-4">&ldquo;{inq.message}&rdquo;</p>
                  <p className="text-[11px] text-white/40">
                    Submitted: {new Date(inq.createdAt).toLocaleString()} | Event:{" "}
        ...
        {activeTab === "stats" && (
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8">
            <div className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit">
              ...
              <form onSubmit={saveHomeStats} className="space-y-5">
                {homeStats.map((stat, index) => (
                  <div key={stat.key || index} className="rounded-lg border border-white/5 bg-black/30 p-4 space-y-4">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-bold uppercase tracking-wider text-gold">Counter {index + 1}</span>
                      <Checkbox
                        label="Active"
                        checked={stat.isActive}
                        onChange={(checked) => updateHomeStat(index, { isActive: checked })}
                      />
                    </div>
                    <TextField
                      label="Label"
                      required
                      value={stat.label}
                      onChange={(value) => updateHomeStat(index, { label: value })}
                    />
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <TextField
                        label="Suffix"
                        value={stat.suffix}
                        onChange={(value) => updateHomeStat(index, { suffix: value })}
                        placeholder="+"
                      />
                    </div>
                    <NumberField
                      ...
                      onChange={(value) => updateHomeStat(index, { sortOrder: value })}
                    />
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 h-fit">
              {homeStats.map((stat) => (
                <div key={stat.key} className="rounded-lg border border-white/5 bg-charcoal p-5">
                  <div className="flex items-center justify-between gap-3">
            ...
            <div className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit">
              ...
              <form onSubmit={saveContactSettings} className="space-y-4">
                <TextField
                  ...
                  onChange={(value) => setContactSettings({ ...contactSettings, phoneLabel: value })}
                />
                <TextField
                  label="Phone Number"
                  required
                  value={contactSettings.phone}
                  onChange={(value) => setContactSettings({ ...contactSettings, phone: value })}
                />
                <TextField
                  label="Phone Link"
                  value={contactSettings.phoneHref}
                  onChange={(value) => setContactSettings({ ...contactSettings, phoneHref: value })}
                  placeholder="tel:+919999999999"
                />
                <TextField
                  label="WhatsApp Number"
                  value={contactSettings.whatsappNumber}
                  onChange={(value) => setContactSettings({ ...contactSettings, whatsappNumber: value })}
                  placeholder="919999999999"
                />
                <TextField
                  label="Email Label"
                  value={contactSettings.emailLabel}
                  onChange={(value) => setContactSettings({ ...contactSettings, emailLabel: value })}
                />
                <TextField
                  label="Email"
                  required
                  value={contactSettings.email}
                  onChange={(value) => setContactSettings({ ...contactSettings, email: value })}
                />
                <TextField
                  label="Address Label"
                  value={contactSettings.addressLabel}
                  onChange={(value) => setContactSettings({ ...contactSettings, addressLabel: value })}
                />
                <TextArea
                  label="Address"
                  required
                ...
                <TextField
                  ...
                  onChange={(value) => setContactSettings({ ...contactSettings, instagramUrl: value })}
                  placeholder="https://www.instagram.com/yourpage"
                />
                <TextField
                  label="Facebook URL"
                  value={contactSettings.facebookUrl}
                  onChange={(value) => setContactSettings({ ...contactSettings, facebookUrl: value })}
                  placeholder="https://www.facebook.com/yourpage"
                />
                <SubmitButton loading={formLoading} label="Save Direct Connect" />
              </form>
            </div>

            <div className="rounded-lg bg-charcoal border border-white/5 p-6 h-fit">
              <h2 className="text-xl font-bold tracking-wide mb-6">Contact Preview</h2>
              <div className="space-y-5">
                {[
                  { label: contactSettings.phoneLabel, value: contactSettings.phone, href: contactSettings.phoneHref },
                  { label: "WhatsApp", value: contactSettings.whatsappNumber, href: `https://wa.me/${contactSettings.whatsappNumber}` },
                  { label: contactSettings.emailLabel, value: contactSettings.email, href: `mailto:${contactSettings.email}` },
                  { label: contactSettings.addressLabel, value: contactSettings.address },
                  ...(contactSettings.instagramUrl ? [{ label: "Instagram", value: contactSettings.instagramUrl, href: contactSettings.instagramUrl }] : []),
                  ...(contactSettings.facebookUrl ? [{ label: "Facebook", value: contactSettings.facebookUrl, href: contactSettings.facebookUrl }] : []),
                ].map((item) => (
                  <div key={item.label} className="rounded-lg border border-white/5 bg-black/30 p-5">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-gold">{item.label}</p>
                    {item.href ? (
                      <a href={item.href} className="mt-2 block text-base font-semibold text-white hover:text-gold">
                        {item.value}
                      </a>
                    ) : (
                      <p className="mt-2 text-sm leading-relaxed text-white/65">{item.value}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        {activeTab === "categories" && (
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8">
            <div ref={categoryFormRef} className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit scroll-mt-32">
              <FormHeader
            ...
            <ItemGrid
              ...
              render={(item) => (
                <ContentCard
                  ...
                  onDelete={() =>
                    deleteItem("/api/categories", item._id, async () => {
                      await Promise.all([fetchCategories(), fetchSubcategories(), fetchGalleryItems()]);
        ...
        {activeTab === "subcategories" && (
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8">
            <div ref={subcategoryFormRef} className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit scroll-mt-32">
              <FormHeader
                title={editingSubcategoryId ? "Update Subcategory" : "Create Subcategory"}
              ...
              />
              <div className="mb-5">
                <Uploader label="Cover Image" accept="image/*" loading={uploadingTarget === "subcategoryCover"} onChange={(e) => handleFileUpload(e, "subcategoryCover")} />
              </div>
              <form onSubmit={saveSubcategory} className="space-y-4">
                <SelectField
                  ...
                  options={activeCategories.map((item) => ({ value: item._id, label: item.name }))}
                />
                <TextField label="Name" required value={subcategoryForm.name} onChange={(value) => setSubcategoryForm({ ...subcategoryForm, name: value })} />
                <TextField label="Slug" value={subcategoryForm.slug} onChange={(value) => setSubcategoryForm({ ...subcategoryForm, slug: value })} placeholder="Auto from name if blank" />
                <TextArea label="Description" value={subcategoryForm.description} onChange={(value) => setSubcategoryForm({ ...subcategoryForm, description: value })} />
                <TextField label="Cover Image URL" value={subcategoryForm.coverImage} onChange={(value) => setSubcategoryForm({ ...subcategoryForm, coverImage: value })} />
                <NumberField label="Sort Order" value={subcategoryForm.sortOrder} onChange={(value) => setSubcategoryForm({ ...subcategoryForm, sortOrder: value })} />
                <Checkbox label="Active" checked={subcategoryForm.isActive} onChange={(checked) => setSubcategoryForm({ ...subcategoryForm, isActive: checked })} />
                <SubmitButton loading={formLoading} label={editingSubcategoryId ? "Update Subcategory" : "Create Subcategory"} />
              </form>
            </div>
            <ItemGrid
              ...
              render={(item) => (
                <ContentCard
                  ...
                  onDelete={() => deleteItem("/api/subcategories", item._id, fetchSubcategories, "Delete this subcategory?")}
                />
              )}
            />
          </div>
        ...
        {activeTab === "gallery" && (
          <div className="grid grid-cols-1 lg:grid-cols-[460px_1fr] gap-8">
            <div ref={galleryFormRef} className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit scroll-mt-32">
              <FormHeader
              ...
              />
              <div className="mb-5">
                <Uploader label="Upload Photo / Video" accept="image/*,video/*" loading={uploadingTarget === "gallery"} onChange={(e) => handleFileUpload(e, "gallery")} />
              </div>
              <form onSubmit={saveGallery} className="space-y-4">
                <TextField label="Title" required value={galleryForm.title} onChange={(value) => setGalleryForm({ ...galleryForm, title: value })} />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <SelectField
                    label="Category"
                    required
                    value={galleryForm.categoryId}
                    onChange={(value) => setGalleryForm({ ...galleryForm, categoryId: value, subcategoryId: "" })}
                    options={activeCategories.map((item) => ({ value: item._id, label: item.name }))}
                  />
                  <SelectField
                    label="Subcategory"
                    value={galleryForm.subcategoryId}
                    onChange={(value) => setGalleryForm({ ...galleryForm, subcategoryId: value })}
                    options={[{ value: "", label: "No subcategory" }, ...filteredSubcategories.map((item) => ({ value: item._id, label: item.name }))]}
                  />
                </div>
                <TextArea label="Description" value={galleryForm.description} onChange={(value) => setGalleryForm({ ...galleryForm, description: value })} />
                <TextField label="SEO Alt Text" value={galleryForm.altText} onChange={(value) => setGalleryForm({ ...galleryForm, altText: value })} placeholder="Cold pyro couple entry in Indore wedding by SKY SFX" />
                <TextArea label="Caption" value={galleryForm.caption} onChange={(value) => setGalleryForm({ ...galleryForm, caption: value })} />
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <TextField label="City" value={galleryForm.city} onChange={(value) => setGalleryForm({ ...galleryForm, city: value })} placeholder="Indore" />
                  <TextField label="Service Slug" value={galleryForm.serviceSlug} onChange={(value) => setGalleryForm({ ...galleryForm, serviceSlug: value })} placeholder="cold-pyro-entry" />
                  <TextField label="Video Duration" value={galleryForm.duration} onChange={(value) => setGalleryForm({ ...galleryForm, duration: value })} placeholder="PT30S" />
                </div>
                <SelectField
                  label="Media Type"
                  value={galleryForm.mediaType}
                  onChange={(value) => setGalleryForm({ ...galleryForm, mediaType: value as "image" | "video", isReel: value === "video" ? galleryForm.isReel : false })}
                  options={[
                    { value: "image", label: "Image" },
                    { value: "video", label: "Video" },
                  ]}
                />
                <TextField label="Media URL" required value={galleryForm.cloudinaryUrl} onChange={(value) => setGalleryForm({ ...galleryForm, cloudinaryUrl: value })} />
                <TextField label="Thumbnail URL" value={galleryForm.thumbnail} onChange={(value) => setGalleryForm({ ...galleryForm, thumbnail: value })} />
                <NumberField label="Sort Order" value={galleryForm.sortOrder} onChange={(value) => setGalleryForm({ ...galleryForm, sortOrder: value })} />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Checkbox label="Featured gallery" checked={galleryForm.featured} onChange={(checked) => setGalleryForm({ ...galleryForm, featured: checked })} />
                  <Checkbox label="Show on home" checked={galleryForm.showOnHome} onChange={(checked) => setGalleryForm({ ...galleryForm, showOnHome: checked })} />
                  <Checkbox label="Use as reel" checked={galleryForm.isReel} disabled={galleryForm.mediaType !== "video"} onChange={(checked) => setGalleryForm({ ...galleryForm, isReel: checked })} />
                  <Checkbox label="Active" checked={galleryForm.isActive} onChange={(checked) => setGalleryForm({ ...galleryForm, isActive: checked })} />
                </div>
                <SubmitButton loading={formLoading} label={editingGalleryId ? "Update Media" : "Publish Media"} />
              </form>
            </div>
            <ItemGrid
              ...
              render={(item) => (
                <ContentCard
                  key={item._id}
                  image={item.mediaType === "video" ? item.thumbnail || item.cloudinaryUrl : item.cloudinaryUrl}
                  ...
                  onDelete={() => deleteItem("/api/gallery", item._id, fetchGalleryItems, "Delete this media item?")}
                />
              )}
            />
          </div>
        ...
        {activeTab === "events" && (
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8">
            <div ref={eventFormRef} className="p-6 rounded-lg bg-charcoal border border-white/5 h-fit scroll-mt-32">
              <FormHeader
                title={editingEventId ? "Update Event Case-study" : "Create Event Case-study"}
              ...
              />
              <div className="mb-5">
                <Uploader label="Upload Event Cover" accept="image/*" loading={uploadingTarget === "event"} onChange={(e) => handleFileUpload(e, "event")} />
              </div>
              <form onSubmit={saveEvent} className="space-y-4">
                <TextField label="Title" required value={eventForm.title} onChange={(value) => setEventForm({ ...eventForm, title: value })} />
                <SelectField
                  label="Category"
            ...
            <ItemGrid
              ...
              render={(item) => (
                <ContentCard
                  ...
                  onDelete={() => deleteItem("/api/events", item._id, fetchEventItems, "Delete this event case-study?")}
                />
              )}
            />
          </div>
...
function Checkbox({
  ...
  return (
    <label className={`flex items-center gap-2 text-sm font-semibold text-white/80 ${disabled ? "opacity-40" : "cursor-pointer"}`}>
      <input
        type="checkbox"
        checked={checked}
        disabled={disabled}
...
function ItemGrid<T>({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">{items.map(render)}</div>;
}
    ...
    <div className="rounded-lg bg-charcoal border border-white/5 overflow-hidden">
      <div className="relative aspect-video bg-black">
        {image ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-white/25 text-xs">No visual</div>
        )}
      ...
      <div className="p-5 space-y-4">
        <div>
          <p className="text-[10px] text-gold uppercase tracking-widest font-bold line-clamp-1">{eyebrow}</p>
          <h3 className="text-white font-bold mt-1 line-clamp-2">{title}</h3>
          <p className="text-white/50 text-xs leading-relaxed mt-2 line-clamp-3">{subtitle}</p>
        </div>
